...
def create_slowfast_nl(
    *,
    ...
    for idx in range(len(stage_depths)):  # stage_depths = (3, 4, 6, 3), 4 stages in total
        ...
        if idx == 1:
            ...
            nonlocal_fast = create_nonlocal(dim_in=64, dim_inner=32)
            stages.append(
                NonlocalLayer(
                    nonlocal_blocks=nn.ModuleList([nonlocal_slow, nonlocal_fast])
                )
            )
...
class NonlocalLayer(nn.Module):
    def __init__(self, nonlocal_blocks):
        super(NonlocalLayer, self).__init__()
        self.blocks = nonlocal_blocks

    def forward(self, x):
        x_out = x
        for idx in range(len(self.blocks)):
            x_out[idx] = self.blocks[idx](x[idx])
        return x_out
